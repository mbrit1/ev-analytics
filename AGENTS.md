...
- `scan_luna` handles focused read-only codebase exploration and evidence gathering; it must not edit files.
...
- Use `scan_luna` only for focused read-only exploration and evidence gathering; it is not an implementation tier and must not receive write work.
