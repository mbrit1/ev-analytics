...
interface RetireLogicalTariffDialogProps {
...
/** Confirms retirement of the current tariff timeline while preserving its history. */
export function RetireLogicalTariffDialog({
  logicalTariffLabel,
  finalActiveDate,
  restoreFocusElement,
  ...
  onCancel,
  onConfirm,
}: RetireLogicalTariffDialogProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="retire-logical-tariff-heading"
        aria-describedby="retire-logical-tariff-description"
        className="w-full max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto"
      >
        <Slab className="w-full space-y-4 p-6">
          <div className="space-y-2">
            <h2 id="retire-logical-tariff-heading" className="text-xl font-semibold text-primary">
              Retire tariff
            </h2>
            <p id="retire-logical-tariff-description" className="text-sm text-secondary">
              <strong>{logicalTariffLabel}</strong>: the final active date is <strong>{finalActiveDateLabel}</strong>.
              Sessions on {finalActiveDateLabel} remain covered. Any future scheduled versions and promotions will be
              {' '}cancelled. This cannot be reversed by editing the historical tariff.
            </p>
          </div>
