...
- **Inline Documentation (JSDoc):**
  - Document all exported interfaces (Props) and components using standard JSDoc (`/** ... */`).
  - Focus on the "why", specific layout behaviors (e.g., mobile-only constraints), and brief prop descriptions. Rely on TypeScript for strict type definitions.
- **Test Documentation:**
  - Use the Arrange, Act, Assert (AAA) pattern via inline comments (`// Arrange: ...`, `// Act: ...`, `// Assert: ...`) inside all test blocks.
  - Include a suite-level JSDoc block above the main `describe` block to explain the overall focus of the test file.
